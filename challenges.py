...
def get_change(amount, coins=eur_coins):
    ...
    `coins` is the set of coins that we need to get change for (i.e. the set of available coins)
    ...
    # lowest by default, so we use `reverse=True` to start with the highest denomination.
